...
export function engineFuelRate() {
	return function (source: Observable<OBDEvent>): Observable<OBDEvent> {
		return source.lift(new EngineFuelRateOperator());
	}
}

class EngineFuelRateOperator implements Operator<OBDEvent, OBDEvent> {
	call(subscriber: Subscriber<OBDEvent>, source: Observable<OBDEvent>): TeardownLogic {
		return source.subscribe(new EngineFuelRateSubscriber(subscriber));
	}
}

class EngineFuelRateSubscriber extends OBDOuterSubscriber {
