...
export function massFlowSensor() {
	return function (source: Observable<OBDEvent>): Observable<OBDEvent> {
		return source.lift(new MassFlowSensorOperator());
	}
}

class MassFlowSensorOperator implements Operator<OBDEvent, OBDEvent> {
	call(subscriber: Subscriber<OBDEvent>, source: Observable<OBDEvent>): TeardownLogic {
		return source.subscribe(new MassFlowSensorSubscriber(subscriber));
	}
}

class MassFlowSensorSubscriber extends OBDOuterSubscriber {
